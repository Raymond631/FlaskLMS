...
def addRoute(app):
    ...
    def allBooks():
        ...
        books = Book.query.paginate(page=times, per_page=10, error_out=False).items

        array_list = []
        for book in books:
            array_list.append(book.to_json())
    ...
    def selectBook():
        ...
        times = request.args.get("times", 1, type=int)
        books = Book.query.filter(Book.name.like("%" + searchKey + "%")).paginate(page=times, per_page=10,
